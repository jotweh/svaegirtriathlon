<?php
// $HeadURL: https://joomgallery.org/svn/joomgallery/JG-2.0/Module/JoomCategories/trunk/helper.php $
// $Id: helper.php 3795 2012-05-21 19:22:17Z erftralle $
/**
...
class modJoomCatHelper extends JoomInterface
{
   ...
   */
  protected function getCats(&$dberror)
  {
    $dberror = '';
    $db      = JFactory::getDBO();
    ...
    $db->setQuery($query, 0, $this->getConfig('categorycount'));
    ...
    if($cat_rows == null)
    {
      if($db->getErrorNum())
      {
        $dberror = JText::_('MOD_JOOMCAT_DB_ERROR_LBL') . ': ' . $db->getErrorMsg();
      }
      else
      {
        $dberror = JText::_('MOD_JOOMCAT_NO_CATEGORIES_FOUND_LBL');
      }
    }
  ...
  protected function getParams($params, $module_id)
  {
    ...
    switch($this->getConfig('showimglink'))
    {
      ...
      case 9:
        ...
        break;
      default:
        // JoomGallery's setting will be used, see interface default setting for 'openimage'
        break;
    }
  ...
  protected function prepareSlideshow(&$imgelement, $slideshowclass, $slideshowid, $catid, $catname, $img_rows)
  {
    ...
    {
      $replace = '<a href="javascript:void(0);" onclick="jsJoomCatModule'.$this->getConfig('module_id').'_'.$catid.'.startSlideshow()">'.$matches[0].'</a>';
